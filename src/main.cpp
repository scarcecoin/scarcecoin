...
#if defined(NDEBUG)
# error "ScarceCoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x1437cecfa9d0236dcea1bd34a0e679d78cfa6c83880069b85cf2217693476e04");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // ScarceCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "ScarceCoin Signed Message:\n";
...
{
    // ScarceCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    {
        // Subsidy is cut in half every 131400 blocks, which will occur approximately every 3 month
...
static const int64 nTargetTimespan = 10 * 60; // ScarceCoin: 10 min
static const int64 nTargetSpacing = 60; // ScarceCoin: 1 minutes
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // ScarceCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool CBlock::AcceptBlock(CValidationState &state, CDiskBlockPos *dbp)
{
    ...
    if (hash != hashGenesisBlock) {
        ...
        // sReject block.nVersion=1 blocks
        if (nVersion < 2)
        {
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x1437cecfa9d0236dcea1bd34a0e679d78cfa6c83880069b85cf2217693476e04");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Robert Downey Jr. - Captain America 3";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1415989071;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1173714623;
        ...
        assert(block.hashMerkleRoot == uint256("0xc5d7113b4afe47fdfa333213dbfc6fba06f95367b9d51532396ba9dea3f98480"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // ScarceCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ScarceCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ScarceCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ScarceCoinMiner(CWallet *pwallet)
{
    printf("ScarceCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("ScarceCoin-miner");
    ...
    try { loop {
        ...
        printf("Running ScarceCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ScarceCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ScarceCoinMiner, pwallet));
}
